...
CipherShare is a secure secret sharing application with dual-layer encryption (AES-256-GCM + PBKDF2). The application enables users to request, submit, and retrieve sensitive information securely. Version 1.1.0 adds optional Cloudflare Turnstile CAPTCHA protection for bot prevention.
...
- **Implement rate limiting** on all API endpoints (multi-tier approach)
- **Use CAPTCHA protection** for sensitive endpoints (optional but recommended)
...
- **Feature Toggle**: Use `CAPTCHA_ENABLED` environment variable (true/false)
- **Protected Endpoints**: Request creation and secret retrieval
- **Provider**: Cloudflare Turnstile (privacy-friendly)
- **Configuration**: Site key (public) and secret key (private)
- **Development**: Can be disabled for easier testing
- **Production**: Recommended to enable for bot protection
- **Implementation**: Utility function `verifyTurnstile()` in `server/utils.ts`
...
- **CAPTCHA_ENABLED**: Set to `false` for local development to skip CAPTCHA
- Set to `true` in production for bot protection
- No code changes needed, purely environment-driven
...
- Update CAPTCHA_CONFIGURATION.md when changing CAPTCHA behavior

## Key Files & Modules

### Backend Services

- `server/crypto.service.ts` - Encryption/decryption logic
- `server/redis.service.ts` - Redis data persistence
- `server/email.service.ts` - Multi-provider email service
- `server/rate-limiters.ts` - Rate limiting configuration
- `server/utils.ts` - Utility functions (CAPTCHA, feature flags)
- `server/types.ts` - TypeScript type definitions
- `server/index.ts` - Express server and routes

### Frontend Components

- `src/components/RequestGeneration.tsx` - Create requests with optional CAPTCHA
- `src/components/SecretSubmission.tsx` - Submit secrets
- `src/components/SecretRetrieval.tsx` - Retrieve secrets with optional CAPTCHA
...
- `docs/CAPTCHA_CONFIGURATION.md` - CAPTCHA setup guide
- `docs/RATE_LIMITING.md` - Rate limiting documentation
- `docs/EMAIL_PROVIDERS.md` - Email provider setup
...
- `SYSTEM_SECRET_KEY` - 64-char hex encryption key
...
- `CF_TURNSTILE_SITEKEY` - Cloudflare Turnstile site key (if CAPTCHA enabled)
- `CF_TURNSTILE_SECRET` - Cloudflare Turnstile secret (if CAPTCHA enabled)
- `CF_TURNSTILE_ALLOWED_HOSTNAMES` - Restrict CAPTCHA to specific domains
